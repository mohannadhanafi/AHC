...
class Registration extends Component {
  state = { serviceTitle: '', serviceSub: '', disable: false  };

  componentWillMount() {
    axios.get('/api/v2/getTitle').then((result) => {
      const { data } = result;
      const { servicetitle, servicesub } = data[0];
      this.setState(() => ({ serviceTitle: servicetitle, serviceSub: servicesub }));
    });
  }
  ...
  render() {
    ...
    const { serviceTitle, serviceSub,disable } = this.state;
    return (
