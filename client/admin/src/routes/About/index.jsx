...
componentDidMount = async () => {
  ...
  const result = await axios('/api/v2/about');
  const { data } = result;
  const {
    title, desc, image, body, sub_title,
  } = data;
  const fileList = [];
  if (image !== '') {
    await axios
  ...
  this.setState({
    title, desc, image, content: body, fileList, sub_title,
  });
}
...
handleEditorChange = (e) => {
          ...
          axios.post('/api/v2/about', {
            ...
            if (status === 200) {
              this.setState({ loading: false }, () => {
                ...
                setTimeout(() => {
                  this.props.history.push('/admin/about/edit');
  ...
  render() {
    ...
    const {
      title, desc, icon, content, fileList, previewVisible, disable, sub_title,
    } = this.state;
    const uploadButton = (
      <div>
        <Icon type="plus" />
    ...
    return (
      <Card className="gx-card" title="Edit About">
