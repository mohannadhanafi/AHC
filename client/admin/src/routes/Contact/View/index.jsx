...
class Dynamic extends React.Component {
  ...
  delete = (id) => {
    axios.delete('/api/v2/contactitems/delete', { data: { id } }).then((res) => {
      const {
        data: { message },
        statusText,
      } = res;
    ...
    componentWillMount = () => {
      axios.get('/api/v2/contactitems').then((result) => {
        ...
        data.map((element) => {
          element.action = (
            <span>
              <Link to={`/admin/contact/${element.id}`} className="icon icon-feedback" />
          ...
          element.title = (
            <Link to={`/admin/contact/${element.id}`}>{element.title}</Link>
          );
          return element;
        });
        this.setState(() => ({ data }));
