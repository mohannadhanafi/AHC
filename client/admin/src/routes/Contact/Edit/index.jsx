...
componentDidMount = async () => {
  ...
  const result = await axios(`/api/v2/contactitems/${id}`);
  const { data } = result;
  const {
...
handleEditorChange = (e) => {
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (fileList.length) {
          axios.post('/api/v2/contactitems/updatecontactitems', {
            data: values,
            params: { id },
          }).then((result) => {
            const {
            ...
            if (status === 200) {
              this.setState({ loading: false }, () => {
                ...
                setTimeout(() => {
                  this.props.history.push('/admin/contact/view');
  ...
  render() {
    ...
    return (
      <Card className="gx-card" title="Edit Contact Item">
